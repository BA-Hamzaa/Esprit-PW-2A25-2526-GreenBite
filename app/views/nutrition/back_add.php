<!-- Vue BackOffice : Ajouter un repas -->
<div style="padding:2rem;max-width:56rem">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      ...
      <p class="text-sm" style="color:var(--text-muted)">Enregistrez un nouveau repas avec ses aliments</p>
    </div>
  </div>
  ...
  <div class="card" style="padding:2rem">
    <form novalidate method="POST" id="repasForm">
      <div class="form-group">
        <label class="form-label" for="nom"><i data-lucide="type" style="width:0.875rem;height:0.875rem"></i> Nom du repas</label>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="form-group">
          ...
          <select name="type_repas" id="type_repas" class="form-input">
            ...
            <option value="petit_dejeuner" <?= (($_POST['type_repas'] ?? '') === 'petit_dejeuner') ? 'selected' : '' ?>>Petit-déjeuner</option>
            <option value="dejeuner" <?= (($_POST['type_repas'] ?? '') === 'dejeuner') ? 'selected' : '' ?>>Déjeuner</option>
            <option value="diner" <?= (($_POST['type_repas'] ?? '') === 'diner') ? 'selected' : '' ?>>Dîner</option>
            <option value="collation" <?= (($_POST['type_repas'] ?? '') === 'collation') ? 'selected' : '' ?>>Collation</option>
          </select>
        </div>
      </div>
      ...
      <div class="mb-6" style="border-top:1px solid var(--border);padding-top:1.5rem">
        ...
        <div id="aliments-container" class="space-y-3">
          <div class="flex gap-3 items-center aliment-row" style="background:var(--muted);padding:0.75rem;border-radius:var(--radius-xl)">
            <select name="aliment_ids[]" class="form-input flex-1"><option value="">-- Aliment --</option><?php foreach ($aliments as $a): ?><option value="<?= $a['id'] ?>"><?= htmlspecialchars($a['nom']) ?> (<?= $a['calories'] ?> kcal)</option><?php endforeach; ?></select>
...
document.getElementById('repasForm').addEventListener('submit', function(e) {
  ...
  if (errors.length > 0) {
    ...
    if (!box) { box = document.createElement('div'); box.id = 'error-box'; box.className = 'p-4 rounded-xl mb-6 flex items-start gap-3 animate-fade-up'; box.style.cssText = 'background:linear-gradient(135deg,#fee2e2,#fef2f2);color:#991b1b;border:1px solid #fca5a5'; document.querySelector('.card').before(box); }
    box.innerHTML = '<div>' + errors.map(e => '<div class="mb-1">⚠️ ' + e + '</div>').join('') + '</div>';
