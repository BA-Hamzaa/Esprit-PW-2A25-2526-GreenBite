                ...
                <td>
                  <div style="display:flex;gap:0.35rem;align-items:center;flex-wrap:wrap">
                    ...
                    <?php if ($isSignale): ?>
                      ...
                      <a href="<?= BASE_URL ?>/?page=admin-comment&action=ignorer&id=<?= $commentId ?>" class="icon-btn" title="Ignorer le signalement" style="background:rgba(82,183,136,0.08);border-color:rgba(82,183,136,0.18);color:var(--primary);text-decoration:none;" onclick="event.preventDefault(); var href=this.href; gbConfirm('Ignorer le signalement ? Le commentaire redeviendra normal.', 'Ignorer', '✅', 'safe').then(ok => { if(ok) window.location.href=href; });">
                    ...
                    <?php else: ?>
                      <!-- Supprimer (simple) -->
                      <a href="<?= BASE_URL ?>/?page=admin-comment&action=delete&id=<?= $commentId ?>" class="icon-btn" title="Supprimer" style="background:rgba(239,68,68,0.06);border-color:rgba(239,68,68,0.15);color:#ef4444;text-decoration:none;" onclick="event.preventDefault(); var href=this.href; gbConfirm('Supprimer ce commentaire définitivement ?', 'Supprimer', '🗑️').then(ok => { if(ok) window.location.href=href; });">
...
function confirmBan(id) {
  gbConfirm(
    'ATTENTION : Cette action supprimera le commentaire et bannira définitivement cet utilisateur.',
    'Supprimer & Bannir',
    '🚫',
    'danger'
  ).then(ok => {
    if (ok) window.location.href = '<?= BASE_URL ?>/?page=admin-comment&action=supprimer-bannir&id=' + id;
  });
}
</script>