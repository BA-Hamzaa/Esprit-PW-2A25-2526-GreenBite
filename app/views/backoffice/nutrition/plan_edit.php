...
<div style="padding:2rem;max-width:56rem;margin:0 auto">
  <a href="<?= BASE_URL ?>/?page=admin-nutrition&action=plans" class="flex items-center gap-2 text-sm mb-6" style="color:var(--secondary);font-weight:500;transition:all 0.3s" onmouseover="this.style.transform='translateX(-4px)'" onmouseout="this.style.transform='translateX(0)'">
...
function sportRowHtml(jour, selected = '', details = '') {
  ...
  const escapedDetails = String(details || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
...
function buildDayCard(jour, duree) {
  ...
  if (existingRepas[jour] && existingRepas[jour].length > 0) {
      existingRepas[jour].forEach(mealId => { mealsHtml += mealRowHtml(jour, mealId); });
  }
  return `
  <div class="day-card" style="background:var(--card);border:1.5px solid var(--border);border-radius:var(--radius-xl);overflow:hidden;transition:all 0.2s">
    ...
    <div style="padding:1.25rem;display:grid;grid-template-columns:1fr;gap:1rem">
      <div>
        ...
        <div class="meals-list" id="meals-day-${jour}">${mealsHtml}</div>
        <button type="button" onclick="addMealToDay(${jour})" style="margin-top:0.4rem;display:inline-flex;align-items:center;gap:0.35rem;padding:0.3rem 0.75rem;background:rgba(82,183,136,0.08);border:1px dashed rgba(82,183,136,0.35);border-radius:0.5rem;color:var(--secondary);font-size:0.75rem;font-weight:600;cursor:pointer;transition:all 0.2s" onmouseover="this.style.background='rgba(82,183,136,0.15)'" onmouseout="this.style.background='rgba(82,183,136,0.08)'">
      ...
      <div>
        ...
        <button type="button" onclick="addSportToDay(${jour})" style="margin-top:0.35rem;display:inline-flex;align-items:center;gap:0.35rem;padding:0.3rem 0.75rem;background:rgba(245,158,11,0.08);border:1px dashed rgba(245,158,11,0.4);border-radius:0.5rem;color:#b45309;font-size:0.75rem;font-weight:600;cursor:pointer;transition:all 0.2s" onmouseover="this.style.background='rgba(245,158,11,0.16)'" onmouseout="this.style.background='rgba(245,158,11,0.08)'">
...
function generateDays(duree) {
  ...
  if (!duree || duree < 1) {
    container.innerHTML = `<div style="padding:1.25rem;background:var(--muted);border:1px dashed var(--border);border-radius:var(--radius-xl);text-align:center;color:var(--text-muted);font-size:0.85rem"><i data-lucide="calendar" style="width:1.5rem;height:1.5rem;display:block;margin:0 auto 0.5rem;opacity:0.4"></i>Entrez la durée du plan ci-dessus pour générer le programme jour par jour.</div>`;
    badge.style.display = 'none';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    return;
  }
  let html = '';
  for (let j = 1; j <= Math.min(duree, 365); j++) html += buildDayCard(j, Math.min(duree, 365));
...
function dayHasMeal(jour) {
  ...
  for (let s of selects) if (s.value && s.value !== '') return true;
  return false;
}
...
function validateSportsDay(jour) {
  ...
  if (!dayHasMeal(jour)) { if (errEl) errEl.style.display = 'none'; return true; }
...
function hydrateSportsFromText(jour, text) {
  ...
  if (!chunks.length) { addSportToDay(jour, 'autre', String(text).trim()); return; }
  chunks.forEach(chunk => {
    const sepIdx = chunk.indexOf(':');
    let typeCandidate = '', details = chunk;
    if (sepIdx !== -1) { typeCandidate = chunk.slice(0, sepIdx).trim().toLowerCase(); details = chunk.slice(sepIdx + 1).trim(); }
...
document.getElementById('duree_jours').addEventListener('input', function(e) { generateDays(parseInt(e.target.value)); });
window.addEventListener('DOMContentLoaded', () => {
  const duree = parseInt(document.getElementById('duree_jours').value);
  if (duree > 0) generateDays(duree);
});
...
function checkLengthRealTime(inputEl, errorId, errorMsg) {
  ...
  if (!errEl) return;
  if (val.length > 0 && val.length < 3) { errEl.innerText = errorMsg; errEl.style.display = 'block'; }
  else errEl.style.display = 'none';
}
function toggleFieldError(errorId, message, shouldShow) {
  const err = document.getElementById(errorId);
  if (!err) return;
  if (shouldShow) { err.innerText = message; err.style.display = 'block'; } else err.style.display = 'none';
}
...
document.getElementById('nom').addEventListener('input', function() { checkLengthRealTime(this, 'err_nom', 'Le nom doit faire au moins 3 caractères.'); });
document.getElementById('description').addEventListener('input', function() { checkLengthRealTime(this, 'err_description', 'La description doit faire au moins 3 caractères.'); });
...
document.getElementById('planForm').addEventListener('submit', function(e) {
  ...
  hideError('err_nom'); hideError('err_description'); hideError('err_type_objectif'); hideError('err_objectif_calories'); hideError('err_duree_jours');
