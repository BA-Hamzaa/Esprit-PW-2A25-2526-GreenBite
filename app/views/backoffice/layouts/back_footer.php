  ...
  <style>
    ...
    .gb-toast.show   { transform: translateX(0); opacity: 1; }
    .gb-toast.hiding { transform: translateX(120%); opacity: 0; }
    .gb-toast.success { background: linear-gradient(135deg,rgba(22,163,74,.95),rgba(16,185,129,.9)); color:#fff; border:1px solid rgba(255,255,255,.2); }
    .gb-toast.error   { background: linear-gradient(135deg,rgba(220,38,38,.95),rgba(239,68,68,.9)); color:#fff; border:1px solid rgba(255,255,255,.2); }
    .gb-toast.warning { background: linear-gradient(135deg,rgba(217,119,6,.97),rgba(245,158,11,.9)); color:#fff; border:1px solid rgba(255,255,255,.2); }
    .gb-toast.info    { background: linear-gradient(135deg,rgba(59,130,246,.97),rgba(99,102,241,.9)); color:#fff; border:1px solid rgba(255,255,255,.2); }
    .gb-toast .gb-toast-icon  { flex-shrink:0; font-size:1.1rem; line-height:1; }
    .gb-toast .gb-toast-close { margin-left:auto; background:transparent; border:none; color:rgba(255,255,255,.75); cursor:pointer; font-size:1rem; line-height:1; padding:0; flex-shrink:0; }
    ...
    #gbConfirmTitle   { font-size:1.05rem; font-weight:800; color:var(--text-primary,#111); margin-bottom:0.4rem; }
    #gbConfirmMessage { font-size:0.875rem; color:var(--text-muted,#6b7280); margin-bottom:1.5rem; line-height:1.55; }
    #gbConfirmActions { display:flex; gap:0.6rem; justify-content:center; }
    #gbConfirmCancel  { padding:0.6rem 1.4rem; border:1.5px solid var(--border,#e5e7eb); border-radius:var(--radius-full,999px); background:transparent; color:var(--text-secondary,#374151); font-size:0.875rem; font-weight:700; cursor:pointer; transition:all 0.2s; }
    #gbConfirmCancel:hover { background:var(--muted,#f3f4f6); }
    #gbConfirmOk { padding:0.6rem 1.5rem; border:none; border-radius:var(--radius-full,999px); background:linear-gradient(135deg,#dc2626,#ef4444); color:#fff; font-size:0.875rem; font-weight:700; cursor:pointer; transition:all 0.2s; box-shadow:0 4px 14px rgba(220,38,38,0.3); }
    #gbConfirmOk:hover { transform:translateY(-1px); box-shadow:0 6px 18px rgba(220,38,38,0.4); }
    #gbConfirmOk.safe { background:linear-gradient(135deg,#2d6a4f,#52b788); box-shadow:0 4px 14px rgba(45,106,79,0.3); }
  ...
  <script>
    ...
    window.gbToast = function(type, msg) {
      ...
      setTimeout(() => { t.classList.add('hiding'); setTimeout(() => t.remove(), 380); }, 4000);
    };
    ...
    window.gbConfirm = function(message, title, icon, okClass) {
      return new Promise(function(resolve) {
        ...
        document.getElementById('gbConfirmOk').className = okClass === 'safe' ? 'safe' : '';
        overlay.classList.add('show');
        function cleanup() { overlay.classList.remove('show'); }
        document.getElementById('gbConfirmOk').onclick     = function() { cleanup(); resolve(true); };
        document.getElementById('gbConfirmCancel').onclick = function() { cleanup(); resolve(false); };
