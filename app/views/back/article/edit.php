      ...
      <div style="margin-top:1rem" class="grid grid-cols-2 gap-4">
        ...
        <div style="display:flex;align-items:flex-end;justify-content:flex-end;gap:0.5rem;flex-wrap:wrap">
          <?php if (($article['statut'] ?? '') !== 'publie'): ?>
            <a href="<?= BASE_URL ?>/?page=admin-article&action=publish&id=<?= (int)$article['id'] ?>" class="btn" style="border-radius:var(--radius-xl);background:rgba(82,183,136,0.08);border:1px solid rgba(82,183,136,0.18);color:var(--primary)" onclick="return confirm('Publier cet article ?')">
