...
function validateDesc() {
  ...
  const val = document.getElementById('raDesc').value.trim();
  if (!val) return showFieldError('raDesc', 'La description est obligatoire.');
  if (val.length > 0 && val.length < 3) return showFieldError('raDesc', 'La description doit contenir au moins 3 caractères.');
  return true;
}

function validateRestr() {
  clearFieldError('raRestrictions');
  const val = document.getElementById('raRestrictions').value.trim();
  if (!val) return showFieldError('raRestrictions', 'Les restrictions sont obligatoires.');
  if (val.length > 0 && val.length < 3) return showFieldError('raRestrictions', 'Les restrictions doivent contenir au moins 3 caractères.');
  return true;
}
