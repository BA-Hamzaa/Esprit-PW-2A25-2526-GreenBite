...
<script>
// ==================== FAVORIS ====================
const GB_SESSION_USER = <?= json_encode($_SESSION['username'] ?? '') ?>;
...
// One-click toggle — no popup needed for logged-in users
function toggleFavori(id) {
  if (GB_SESSION_USER) {
    // Logged in: toggle immediately
    _doToggleFavori(id, GB_SESSION_USER);
  } else {
    // Guest: just show a small name prompt (no PIN)
    showFavoriPopup(id);
  }
}
...
// Guest fallback — name only, no PIN
...
function shareArticle(id, titre) {
  ...
  popup.querySelector('#copy-link-btn').onclick = function() {
    navigator.clipboard.writeText(url).then(function() {
      gbToast('success', '📋 Lien copié dans le presse-papier !');
      popup.remove();
    });
  };
