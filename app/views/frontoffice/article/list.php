...
<div style="padding:2.5rem">
  ...
  <?php else: ?>
    ...
    <div class="grid grid-cols-2 gap-5">
      ...
      ?>
        <div class="card"
           style="padding:1.8rem;display:block;position:relative;
                  border-left:5px solid <?= $accent['border'] ?>;
                  background:linear-gradient(135deg, var(--card) 0%, <?= $accent['bg'] ?> 100%);">

          <!-- ⭐ FAVORI (bigger) -->
          <button onclick="event.preventDefault(); event.stopPropagation(); toggleFavori(<?= $articleId ?>)" id="favori-btn-<?= $articleId ?>" style="position:absolute;top:0.6rem;right:3.2rem;background:transparent;border:none;cursor:pointer;font-size:1.5rem;color:var(--text-muted);padding:0.2rem;line-height:1;transition:all 0.2s;z-index:5" title="Ajouter aux favoris">☆</button>

          <!-- 📤 SHARE -->
          <button onclick="event.preventDefault(); event.stopPropagation(); shareArticle(<?= $articleId ?>, '<?= addslashes(htmlspecialchars($a['titre'] ?? '')) ?>')" style="position:absolute;top:0.6rem;right:0.6rem;display:flex;align-items:center;justify-content:center;width:2.2rem;height:2.2rem;border-radius:50%;border:1px solid var(--border);background:var(--surface);cursor:pointer;transition:all 0.2s;color:var(--text-muted);z-index:5" title="Partager">
          ...
          <div style="margin-top:0.6rem;display:flex;align-items:center;gap:0.5rem;border-top:1px solid var(--border);padding-top:0.6rem;padding-bottom:2rem">
...
<!-- PAGINATION -->
<?php if ($totalPages > 1): ?>
<div style="display:flex;justify-content:center;align-items:center;gap:0.4rem;margin-top:2rem;flex-wrap:wrap">
  <?php if ($page > 1): ?>
    <a href="<?= BASE_URL ?>/?page=article&action=list&p=<?= $page - 1 ?><?= $keyword ? '&q=' . urlencode($keyword) : '' ?>" class="btn" style="border-radius:var(--radius-full);padding:0.4rem 0.9rem;font-size:0.8rem;background:rgba(45,106,79,0.06);border:1px solid rgba(45,106,79,0.15);color:var(--primary);text-decoration:none;">← Précédent</a>
  <?php endif; ?>
  <?php for ($i = 1; $i <= $totalPages; $i++): ?>
    <a href="<?= BASE_URL ?>/?page=article&action=list&p=<?= $i ?><?= $keyword ? '&q=' . urlencode($keyword) : '' ?>" class="btn" style="border-radius:var(--radius-full);padding:0.4rem 0.7rem;font-size:0.8rem;<?= $i === $page ? 'background:var(--primary);color:#fff;border:none;font-weight:700;' : 'background:rgba(45,106,79,0.04);border:1px solid var(--border);color:var(--text-muted);' ?>text-decoration:none;"><?= $i ?></a>
  <?php endfor; ?>
  <?php if ($page < $totalPages): ?>
    <a href="<?= BASE_URL ?>/?page=article&action=list&p=<?= $page + 1 ?><?= $keyword ? '&q=' . urlencode($keyword) : '' ?>" class="btn" style="border-radius:var(--radius-full);padding:0.4rem 0.9rem;font-size:0.8rem;background:rgba(45,106,79,0.06);border:1px solid rgba(45,106,79,0.15);color:var(--primary);text-decoration:none;">Suivant →</a>
  <?php endif; ?>
</div>
<?php endif; ?>

<script>
// ==================== FAVORIS (with PIN popup) ====================
...
function showFavoriPopup(id) {
  ...
  popup.style.cssText = `
    position:fixed;
    top:${Math.min(rect.top - 10, window.innerHeight - 250)}px;
    left:${Math.max(10, rect.left - 150)}px;
    background:var(--card);
  ...
  setTimeout(function() {
    const nameInput = document.getElementById('fav-name-' + id);
...
function saveFavoriWithPin(id) {
  ...
  if (name.length < 2) {
    if (err) { err.textContent = 'Nom requis (min 2 caractères)'; err.style.display = 'block'; }
    return;
  }
  if (!/^\d{4}$/.test(pin)) {
    if (err) { err.textContent = 'PIN requis (4 chiffres)'; err.style.display = 'block'; }
    return;
  }
  ...
  if (idx === -1) {
    favoris.push(id);
    if (btn) { btn.innerHTML = '⭐'; btn.style.color = '#f59e0b'; }
  } else {
    favoris.splice(idx, 1);
    if (btn) { btn.innerHTML = '☆'; btn.style.color = 'var(--text-muted)'; }
  }
...
function loadFavoriStates() {
  const allStars = document.querySelectorAll('[id^="favori-btn-"]');
  allStars.forEach(function(btn) {
    btn.innerHTML = '☆';
...
function shareArticle(id, titre) {
  ...
  popup.innerHTML = `
    <div style="background:var(--card);border-radius:1.5rem;padding:2rem;max-width:340px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3)">
      <div style="font-weight:800;margin-bottom:1rem;color:var(--text-primary)">📤 Partager cet article</div>
      <div style="display:flex;flex-direction:column;gap:0.5rem">
        ...
        <button id="close-share-btn" style="width:100%;padding:0.7rem;border:none;border-radius:0.75rem;background:rgba(0,0,0,0.05);cursor:pointer;font-size:0.85rem;color:var(--text-muted);margin-top:0.5rem">Fermer</button>
      </div>
    </div>
  `;
  document.body.appendChild(popup);

  popup.querySelector('#copy-link-btn').onclick = function() {
    navigator.clipboard.writeText(url).then(function() {
      alert('Lien copié !');
      popup.remove();
    });
  };
  popup.querySelector('#wa-btn').onclick = function() {
    window.open('https://wa.me/?text=' + encodeURIComponent(titre + ' ' + url), '_blank');
    popup.remove();
  };
  popup.querySelector('#fb-btn').onclick = function() {
...
if (file_exists($citationsFile)) {
    require_once $citationsFile;
}
...
">
  <div style="
    ...
    animation:hippoPopIn 0.5s cubic-bezier(0.34,1.56,0.64,1);
  ">
    ...
    <p style="
      ...
      padding:0 0.5rem;
    ">
      "<?= htmlspecialchars($citation['texte']) ?>"
    </p>

    <img src="<?= BASE_URL ?>/assets/images/memes/<?= htmlspecialchars($citation['image']) ?>"
         alt="meme"
         style="width:100%;max-width:340px;height:auto;max-height:340px;object-fit:contain;border-radius:1.5rem;margin-bottom:1.5rem;background:rgba(0,0,0,0.02)" />
...
.sparkle {
  ...
  box-shadow:0 0 8px 2px rgba(82,183,136,0.6);
}
@keyframes exploseParticle {
...
.explosion-particle {
  ...
  pointer-events:none;
  z-index:10001;
  animation:exploseParticle 0.6s ease-out forwards;
  background:var(--color, #52b788);
  box-shadow:0 0 12px 4px var(--color, rgba(82,183,136,0.7));
}
...
<script>
(function() {
  const btn   = document.getElementById('hippoBtn');
  ...
  let vx = (Math.random() > 0.5 ? 1 : -1) * (0.35 + Math.random() * 0.3);
  let vy = (Math.random() > 0.5 ? 1 : -1) * (0.35 + Math.random() * 0.3);
  let moving    = true;
  let captured  = false;
  let rafId     = null;
  let countdown = 30;
  ...
  btn.addEventListener('click', function() {
    ...
    setTimeout(function() { document.body.classList.remove('shake-active'); }, 500);

    setTimeout(function() {
      openModal();
      btn.style.transition = 'left 0.8s ease, top 0.8s ease, opacity 0.3s, transform 0.3s';
      btn.style.opacity = '1';
      btn.style.transform = 'scale(0.7)';
      btn.style.fontSize = '3rem';
      btn.style.left = (window.innerWidth  - 90) + 'px';
      btn.style.top  = (window.innerHeight - 90) + 'px';
    }, 300);
  });
  ...
  function loadLike() {
    const count = parseInt(localStorage.getItem(LIKE_KEY) || '0');
    document.getElementById('likeCount').textContent = count;
    const liked = localStorage.getItem(LIKE_KEY + '_liked') === '1';
    const likeBtn = document.getElementById('likeBtn');
    if (liked) {
      likeBtn.style.background = 'rgba(22,163,74,0.12)';
      likeBtn.style.borderColor = '#86efac';
      likeBtn.style.color = '#16a34a';
    }
  }
  ...
  window.toggleLike = function() {
    ...
    let count = parseInt(localStorage.getItem(LIKE_KEY) || '0');

    if (liked) {
      count = Math.max(0, count - 1);
      localStorage.setItem(LIKE_KEY, count);
      localStorage.removeItem(LIKE_KEY + '_liked');
      likeBtn.style.background = 'transparent';
      likeBtn.style.borderColor = 'var(--border)';
      ...
      count++;
      localStorage.setItem(LIKE_KEY, count);
      localStorage.setItem(LIKE_KEY + '_liked', '1');
      likeBtn.style.background = 'rgba(22,163,74,0.12)';
      likeBtn.style.borderColor = '#86efac';
      likeBtn.style.color = '#16a34a';
    }
