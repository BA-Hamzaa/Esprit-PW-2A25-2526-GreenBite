    ...
    <div class="grid grid-cols-2 gap-0">
      ...
      <div style="height:22rem;background:var(--muted);overflow:hidden">
        <?php if (!empty($produit['image'])): ?>
          <img src="<?= BASE_URL ?>/assets/images/<?= htmlspecialchars($produit['image']) ?>" alt="<?= htmlspecialchars($produit['nom']) ?>" style="width:100%;height:100%;object-fit:cover">
        <?php else: ?>
          <div style="height:100%;background:linear-gradient(135deg, #dcfce7, #f0fdf4);display:flex;align-items:center;justify-content:center"><i data-lucide="image" style="width:4rem;height:4rem;color:#86efac"></i></div>
        <?php endif; ?>
      </div>
