        ...
        <div class="card hover-shadow card-interactive" style="padding:0;overflow:hidden;border:none">
          <div style="height:10rem;background:var(--muted);position:relative;overflow:hidden">
            <?php if (!empty($p['image'])): ?>
              <img src="<?= BASE_URL ?>/assets/images/<?= htmlspecialchars($p['image']) ?>" alt="<?= htmlspecialchars($p['nom']) ?>" style="width:100%;height:100%;object-fit:cover;transition:transform 0.5s" onmouseover="this.style.transform='scale(1.08)'" onmouseout="this.style.transform='scale(1)'">
            <?php else: ?>
              <div style="height:100%;background:linear-gradient(135deg, #dcfce7, #f0fdf4);display:flex;align-items:center;justify-content:center"><i data-lucide="image" style="width:2.5rem;height:2.5rem;color:#86efac"></i></div>
            <?php endif; ?>
