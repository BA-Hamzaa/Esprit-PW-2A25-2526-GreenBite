<!-- Vue BackOffice : Ajouter un produit -->
<div style="padding:2rem;max-width:56rem">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      <h1 class="text-2xl font-bold" style="color:var(--text-primary);font-family:var(--font-heading)">Ajouter un Produit</h1>
      <p class="text-sm" style="color:var(--text-muted)">Remplissez les informations du nouveau produit</p>
    </div>
  </div>
  ...
  <div class="card" style="padding:0;overflow:hidden">
    ...
    <div style="display:flex;border-bottom:1px solid var(--border)">
      <div class="form-step active" data-step="1" style="flex:1;padding:1rem;text-align:center;cursor:pointer;transition:all 0.3s;border-bottom:3px solid var(--primary);color:var(--primary);font-weight:600;font-size:0.85rem">
        <i data-lucide="package" style="width:1rem;height:1rem;display:inline;vertical-align:middle"></i> Informations
      </div>
      <div class="form-step" data-step="2" style="flex:1;padding:1rem;text-align:center;cursor:pointer;transition:all 0.3s;border-bottom:3px solid transparent;color:var(--text-muted);font-weight:500;font-size:0.85rem">
        <i data-lucide="settings-2" style="width:1rem;height:1rem;display:inline;vertical-align:middle"></i> Détails
      </div>
      <div class="form-step" data-step="3" style="flex:1;padding:1rem;text-align:center;cursor:pointer;transition:all 0.3s;border-bottom:3px solid transparent;color:var(--text-muted);font-weight:500;font-size:0.85rem">
    ...
    <form novalidate method="POST" enctype="multipart/form-data" id="produitForm" style="padding:2rem">
      ...
      <div id="step1">
        ...
        <div class="form-group">
          ...
          <input type="text" name="nom" id="nom" class="form-input" placeholder="Ex: Avocat Bio de Bizerte" value="<?= htmlspecialchars($_POST['nom'] ?? '') ?>" style="padding:0.85rem 1rem;border-radius:var(--radius-xl)">
          <span class="text-xs" style="color:var(--text-muted);margin-top:4px;display:block">Min. 3 caractères</span>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="form-group">
            ...
            <select name="categorie" id="categorie" class="form-input" style="padding:0.85rem 1rem;border-radius:var(--radius-xl)">
              ...
              foreach ($cats as $c): ?>
                <option value="<?= $c ?>" <?= (isset($_POST['categorie']) && $_POST['categorie'] === $c) ? 'selected' : '' ?>><?= ucfirst($c) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
            <label class="form-label" for="producteur"><i data-lucide="store" style="width:0.75rem;height:0.75rem"></i> Producteur</label>
            <input type="text" name="producteur" id="producteur" class="form-input" placeholder="Ferme El Khir..." value="<?= htmlspecialchars($_POST['producteur'] ?? '') ?>" style="padding:0.85rem 1rem;border-radius:var(--radius-xl)">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label" for="description"><i data-lucide="align-left" style="width:0.75rem;height:0.75rem"></i> Description</label>
          <textarea name="description" id="description" class="form-textarea" rows="3" placeholder="Décrivez le produit, son origine et ses qualités..." style="border-radius:var(--radius-xl)"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
        </div>
      </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="form-group">
            ...
            <div style="position:relative">
              <input type="number" name="prix" id="prix" class="form-input" step="0.01" placeholder="4.50" value="<?= htmlspecialchars($_POST['prix'] ?? '') ?>" style="padding:0.85rem 1rem;border-radius:var(--radius-xl);padding-right:3rem">
          ...
          <div class="form-group">
            ...
            <input type="number" name="stock" id="stock" class="form-input" placeholder="100" value="<?= htmlspecialchars($_POST['stock'] ?? '0') ?>" style="padding:0.85rem 1rem;border-radius:var(--radius-xl)">
          </div>
          <div class="form-group">
            <label class="form-label"><i data-lucide="leaf" style="width:0.75rem;height:0.75rem"></i> Produit Bio ?</label>
            <div style="display:flex;align-items:center;gap:0.75rem;padding:0.85rem 1rem;background:var(--muted);border-radius:var(--radius-xl);border:1px solid var(--border)">
              <label class="flex items-center gap-2" style="cursor:pointer;font-size:0.875rem;color:var(--text-primary)">
                <input type="radio" name="is_bio" value="1" <?= (isset($_POST['is_bio']) && $_POST['is_bio'] === '1') ? 'checked' : '' ?> style="accent-color:var(--primary)"> 🌱 Oui
              </label>
              <label class="flex items-center gap-2" style="cursor:pointer;font-size:0.875rem;color:var(--text-primary)">
                <input type="radio" name="is_bio" value="0" <?= (!isset($_POST['is_bio']) || $_POST['is_bio'] === '0') ? 'checked' : '' ?> style="accent-color:var(--primary)"> Non
              </label>
            </div>
          </div>
        </div>
      ...
      <div id="step3">
        <h3 class="font-semibold mb-4 flex items-center gap-2" style="color:var(--text-primary)">
          <i data-lucide="image" style="width:1rem;height:1rem;color:#7c3aed"></i> Image du produit
        </h3>
        <div id="dropzone" style="border:2px dashed var(--border);border-radius:var(--radius-xl);padding:3rem 2rem;text-align:center;cursor:pointer;transition:all 0.3s;background:var(--muted)" onmouseover="this.style.borderColor='var(--secondary)';this.style.background='var(--surface)'" onmouseout="this.style.borderColor='var(--border)';this.style.background='var(--muted)'" onclick="document.getElementById('image').click()">
          <i data-lucide="cloud-upload" style="width:2.5rem;height:2.5rem;color:var(--text-muted);display:inline-block;margin-bottom:0.75rem"></i>
          <p class="font-medium" style="color:var(--text-primary);margin-bottom:0.25rem">Glissez une image ici</p>
          <p class="text-xs" style="color:var(--text-muted)">ou cliquez pour parcourir • JPG, PNG, max 2MB</p>
          <div id="preview" style="margin-top:1rem;display:none">
            <img id="previewImg" style="max-height:8rem;border-radius:var(--radius-lg);margin:0 auto;box-shadow:0 4px 15px rgba(0,0,0,0.1)">
          </div>
      ...
      <div style="display:flex;gap:0.75rem;margin-top:2rem">
        <button type="submit" class="btn btn-primary btn-lg flex-1" style="border-radius:var(--radius-xl);padding:0.9rem">
          <i data-lucide="save" style="width:1.125rem;height:1.125rem"></i> Enregistrer le produit
        </button>
