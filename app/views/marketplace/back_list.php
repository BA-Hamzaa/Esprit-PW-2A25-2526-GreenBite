          ...
          <?php else: ?>
            <?php foreach ($produits as $p): ?>
              <tr>
                ...
                <td><div style="width:3rem;height:3rem;border-radius:var(--radius-lg);overflow:hidden;background:var(--muted)"><?php if (!empty($p['image'])): ?><img src="<?= BASE_URL ?>/assets/images/<?= htmlspecialchars($p['image']) ?>" style="width:100%;height:100%;object-fit:cover"><?php else: ?><div style="width:100%;height:100%;background:linear-gradient(135deg, #dcfce7, #f0fdf4);display:flex;align-items:center;justify-content:center"><i data-lucide="image" style="width:1.25rem;height:1.25rem;color:#86efac"></i></div><?php endif; ?></div></td>
