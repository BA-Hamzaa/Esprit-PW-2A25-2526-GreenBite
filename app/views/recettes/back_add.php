...
<div style="padding:2rem;max-width:56rem">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      <h1 class="text-2xl font-bold" style="color:var(--text-primary);font-family:var(--font-heading)">Ajouter une Recette</h1>
      <p class="text-sm" style="color:var(--text-muted)">Créez une nouvelle recette durable</p>
    </div>
  </div>
...
document.getElementById('recetteForm').addEventListener('submit', function(e) {
  ...
  if (errors.length > 0) { e.preventDefault(); let box = document.getElementById('error-box'); if (!box) { box = document.createElement('div'); box.id='error-box'; box.className='p-4 rounded-xl mb-6 flex items-start gap-3'; box.style.cssText='background:linear-gradient(135deg,#fee2e2,#fef2f2);color:#991b1b;border:1px solid #fca5a5'; document.querySelector('.card').before(box); } box.innerHTML = '<div>' + errors.map(e=>'<div class="mb-1">⚠️ '+e+'</div>').join('') + '</div>'; box.scrollIntoView({behavior:'smooth'}); }
});
</script>
