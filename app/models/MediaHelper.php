<?php
/**
 * MediaHelper — Résolution des URLs d'images uploadées + fallback Unsplash.
 * Classe utilitaire statique, part of the Model layer.
 */
class MediaHelper
{
    public static function uploadsAbsDir(): string
    {
        return BASE_PATH . '/app/views/public/assets/images/uploads';
    }

    public static function fallbackRecette(?string $categorie): string
    {
        $map = [
    ...
    public static function fallbackProduit(?string $categorie): string
    {
        $map = [
    ...
    /**
     * URL finale pour affichage : fichier local si présent, sinon URL absolue déjà en BDD, sinon fallback.
     *
     * @param string|null $stored  Valeur colonne image (nom fichier ou URL)
     * @param string      $fallback URL photo de repli (Unsplash)
     */
    public static function url(?string $stored, string $fallback): string
    {
        ...
        if ($stored !== '') {
            ...
            $path = self::uploadsAbsDir() . DIRECTORY_SEPARATOR . $name;
            if (is_file($path)) {
